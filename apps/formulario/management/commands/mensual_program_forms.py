...
class Command(BaseCommand):
    """
        ...
        debe ejecutarse el primero de mes
    """
    help = u'Check if a form has to be CREATED'

    def handle(self, *args, **options):
        today = datetime.now()
        period_model = Period.objects.get(code='MO') #obtain model of period
        last_form = today - relativedelta(months=period_model.months) #ojo acá
