...
from ..serializers.actor_type_serializer import ActorTypeSerializer, ActorTypeSerializerDetail, ActorTypeSerializerSimpleName
...
class ActorTypeResource(resources.ModelResource):
    ...
    def dehydrate_name_en(self, obj):
        ...
        return getattr(obj, name_translate)
...
class ActorTypeList(generics.ListAPIView):
    serializer_class = ActorTypeSerializerDetail

    def get_queryset(self):
        queryset = ActorType.objects.all()
        ...
        if search != None:
            fields = [
                ...
                'proforest_actortype_code__icontains',
            ]
            filters = Q()
...
@permission_classes([CustomDjangoModelPermission])
class ActorTypeCreate(generics.CreateAPIView):
    queryset = ActorType.objects.all()
    serializer_class = ActorTypeSerializer


@permission_classes([CustomDjangoModelPermission])
class ActorTypeView(generics.RetrieveAPIView):
    queryset = ActorType.objects.all()
    serializer_class = ActorTypeSerializerDetail


@permission_classes([CustomDjangoModelPermission])
class ActorTypeUpdate(generics.UpdateAPIView):
    queryset = ActorType.objects.all()
    serializer_class = ActorTypeSerializer
    def perform_update(self, serializer ):
       
        actortype = ActorType.objects.get(id = self.request.data['id'])
        commodity = actortype.commodity
        if commodity.status == False:
            raise serializers.ValidationError("The commodity asociated to this Actor-Type is disabled cannot be updated")
        else:
            serializer.save(
                is_productor = self.request.data['isProductor']['value']
            )


@permission_classes([CustomDjangoModelPermission])
class ActorTypeDestroy(generics.DestroyAPIView):
    queryset = ActorType.objects.all()
    serializer_class = ActorTypeSerializer
                    ...
                    "errors": [
                        {
                            ...
                            "detail": "ActorType cannot be deleted it is asociated to other models.",
                            "attr": "ActorType"
                        }
                    ]
                },
...
class ActorTypeSearch(APIView):
    ...
    def get(self, request):
        ...
        if search != None:
            ...
            for str_field in fields:
                filters |= Q(**{str_field: search})
            queryset = ActorType.objects.filter(filters).order_by('id')[:5]
        
        else:
            queryset = ActorType.objects.all()[:5]

        array = []
        for actortype in queryset:
            array.append(getattr(actortype, 'name_en'))
            array.append(getattr(actortype, 'name_es'))
            array.append(getattr(actortype, 'name_pt'))
...
@method_decorator(permission_required('company.view_actortype', raise_exception=True), name='dispatch')
class ActorTypeListView(ListView):
    template_name = 'company/actor_type/actor_type_list.html'
    url_name = 'actor-type-list'
    model = ActorType
    paginate_by = 25
    ...
    def get_queryset(self):
        ...
        queryset = ActorType.objects.filter(filters).order_by('-id')
...
@method_decorator(login_required, name='dispatch')
@method_decorator(permission_required('company.add_actortype', raise_exception=True), name='dispatch')
class ActorTypeCreateView(CreateView):
    template_name = 'company/actor_type/actor_type_create.html'
    url_name = 'actor-type-create'
    model = ActorType
    fields = [
        'commodity', 'name_es', 'name_en', 'name_pt', 'proforest_actortype_code',
        'is_productor'
    ]

    def get_success_url(self):
        return reverse_lazy("actor-type-list")
...
@method_decorator(login_required, name='dispatch')
@method_decorator(permission_required('company.change_actortype', raise_exception=True), name='dispatch')
class ActorTypeUpdateView(UpdateView):
    template_name = 'company/actor_type/actor_type_update.html'
    url_name = 'actor-type-update'
    model = ActorType
    fields = [
        'name_es', 'name_en', 'name_pt', 'proforest_actortype_code', 'status',
        'is_productor'
    ]
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        context['nav_actor_type'] = True
        context['commodity'] = getattr(self.get_object(), name_translate)
        return context
...
@method_decorator(login_required, name='dispatch')
@method_decorator(permission_required('company.change_actortype', raise_exception=True), name='dispatch')
class ActorTypeDeactivateView(View):
    url_name = 'actor-type-deactivate'

    def post(self, request,):
        actor_type = get_object_or_404(ActorType, pk=self.request.POST['pk'])
        message = _('Activation')
        if (actor_type.status):
            actor_type.status = False
            message = _('Deactivation')
            actor_type.save()
        else:
            actor_type.status = True
            actor_type.save()
...
# =============================================================================
@method_decorator(login_required, name='dispatch')
@method_decorator(permission_required('company.delete_actortype', raise_exception=True), name='dispatch')
class ActorTypeDeleteView(View):
    url_name = 'actor-type-delete'
...
@method_decorator(login_required, name='dispatch')
@method_decorator(permission_required('company.view_actortype', raise_exception=True), name='dispatch')
class ActorTypeExportView(View):
    url_name = 'actor-type-export'

    def _get_pdf_export(self, queryset, file_name):
        template = get_template('report/actor_type/actor_type_list.html')
        context = {
    ...
    def _get_export_response(self, queryset, format='csv'):
        dataset = ActorTypeResource().export(queryset)
        file_name = "actortype_{}".format(dt.datetime.now().strftime("%d-%m-%Y"))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (selected_all_elements == '1' or selected_all_elements == 1):
            queryset = ActorType.objects.all().order_by('-id')
            ...
            if not (id_seleccion == '' or id_seleccion == None):
                queryset = ActorType.objects.filter(
...
class ActorTypeCommodityList(generics.ListAPIView):
    serializer_class = ActorTypeSerializer
    queryset = ActorType.objects.all()
    permission_classes = []
    ...
    def get_queryset(self):
        return ActorType.objects.filter(commodity__pk=self.kwargs.get('id_commodity'))
# =============================================================================
